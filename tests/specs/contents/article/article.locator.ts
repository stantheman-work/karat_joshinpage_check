...
async function getCreatearticleCommunityAssigned(page: Page) {
  const createarticleCommunityAssigned = page.locator("//div[@role='radiogroup']/label/span[1]")
  const count = await createarticleCommunityAssigned.count()

  let textFieldArray: Locator[] = []
  for (let i = 0; i < count; i++) {
    textFieldArray.push(createarticleCommunityAssigned.nth(i))
  }
  console.log("This is the value of Create article community assigned = " + textFieldArray.length + "\n")
  return textFieldArray
}

async function getCreatearticleSpecificcommunityDropdown(page) {
  const createarticleSpecificcommunityDropdown = page.locator("//input[@name='community']")
  await expect(createarticleSpecificcommunityDropdown).toBeVisible({ timeout: 5000 })
  return createarticleSpecificcommunityDropdown
}
...
async function getCreateArticleThumbnail(page) {
  const createArticleThumbnail = page.locator("//input[@accept]/parent::div")
