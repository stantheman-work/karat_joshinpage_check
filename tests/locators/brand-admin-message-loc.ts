...
async function getMessageTokenandRecipient(page: Page) {
  const messageTokenandRecipient = page.locator("//div[@role='combobox']")
  const count = await messageTokenandRecipient.count()

  let textFieldArray: Locator[] = []
  for (let i = 0; i < count; i++) {
    textFieldArray.push(messageTokenandRecipient.nth(i))
  }
  console.log("This is the value of Message token and recipient = " + textFieldArray.length + "\n")
  return textFieldArray
}
...
async function getMessageTokenDropdown(page: Page) {
  const messageTokenDropdown = page.locator("//li[@role='option']/div")
  const count = await messageTokenDropdown.count()

  let textFieldArray: Locator[] = []
  for (let i = 0; i < count; i++) {
    textFieldArray.push(messageTokenDropdown.nth(i))
  }
  console.log("This is the value of Message token dropdown = " + textFieldArray.length + "\n")
  return textFieldArray
}
