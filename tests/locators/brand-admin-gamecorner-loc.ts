...
async function getGamecornerTokenFilter(page) {
  const gamecornerTokenFilter = page.locator("//label[text()='Token']/following-sibling::div/div")
  await expect(gamecornerTokenFilter).toBeVisible({ timeout: 5000 })
  return gamecornerTokenFilter
}
...
async function getCreategameTokenDropdown(page) {
  const creategameTokenDropdown = page.locator(
    "//h6[contains(text(),'Token')]/parent::div/following-sibling::div/descendant::div[@role]"
  )
  await expect(creategameTokenDropdown).toBeVisible({ timeout: 5000 })
  return creategameTokenDropdown
}
